...
use crate::models::{CacheKey, Saying, SayingSource};
...
impl Storage {
    pub fn new(config: StorageConfig) -> Self {
        let inner = match config.type_ {
            ...
            StorageType::Sled => match SledStorage::new(&config.connection_string) {
    ...
    pub async fn find_cached_saying(
        &self,
        prompt: &str,
        preset_id: Option<&str>,
    ) -> Result<Option<Saying>> {
        match &self.inner {
...
impl MemoryStorage {
    ...
    fn find_cached_saying(&self, prompt: &str, preset_id: Option<&str>) -> Result<Option<Saying>> {
        ...
        let cache_key = CacheKey::new(preset_id.map(|id| id.to_string()), prompt.to_string());
            ...
            for user_sayings in sayings_map.values() {
                for saying in user_sayings {
                    if !matches!(saying.source, SayingSource::LLM) {
                        ...
                        let is_duplicate = all_cached_sayings
                            .iter()
                            .any(|s| s.prompt == saying.prompt && s.preset_id == saying.preset_id);
...
impl SledStorage {
    fn new(path: &str) -> Result<Self> {
        ...
        db.open_tree("global_cache")
            .context("Failed to create global cache tree")?;
    ...
    fn save_saying(&self, user_id: &str, saying: Saying) -> Result<Saying> {
        ...
        self.db
            .insert(user_id.as_bytes(), serialized)
            .context("Failed to insert into Sled database")?;

        // Add to global cache if it's not an LLM source
        if !matches!(saying.source, SayingSource::LLM) {
            let global_tree = self
                .db
                .open_tree("global_cache")
                .context("Failed to open global cache tree")?;

            // Create a unique key based on preset + prompt
            let cache_key = CacheKey::from_saying(&saying);
            let key_bytes =
                serde_json::to_vec(&cache_key).context("Failed to serialize cache key")?;

            // Store the saying in the global cache
            let serialized_saying =
                serde_json::to_vec(&saying).context("Failed to serialize saying for cache")?;
            global_tree
                .insert(key_bytes, serialized_saying)
                .context("Failed to insert into global cache")?;
        }
    ...
    fn find_cached_saying(&self, prompt: &str, preset_id: Option<&str>) -> Result<Option<Saying>> {
        ...
        for result in self.db.iter() {
            ...
            let sayings: Vec<Saying> =
                serde_json::from_slice(&ivec).context("Failed to deserialize sayings from Sled")?;
...
mod tests {
    ...
    fn test_memory_storage_find_cached_saying() {
        ...
        let result = storage
            .find_cached_saying(prompt, preset_id.as_deref())
            .unwrap();
        ...
        let no_result = storage
            .find_cached_saying("nonexistent", preset_id.as_deref())
            .unwrap();
